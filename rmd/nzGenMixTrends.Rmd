...
Figure \@ref(fig:meanGBrenewablesVsfossilHalfHourPC) shows the mean half-hourly % generation by each type per day. This is slightly convoluted - it is the mean of the sum of the 48 daily half-hourly values. Unfold the code above for clarity.
...
Instead use a model derived from the GB MWh generation mix (which may be way off but...)
...
Even more amusingly it creates large negative numbers - probably due to the dominance of Hydro...
...
# 2020 value
nzGenMix_wide_dt[, CARBON_INTENSITY := 234.426 + (Gas * 0.002377) +
                   (Coal * 0.016464) +
                   (Wind * -0.009415) +
                   (Hydro * -0.013780) +
                   (Solar * -0.007924)]
