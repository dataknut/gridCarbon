...
# local functions ----
fix_dateTime_axis <- function(p, breaks = "3 months"){
  p <- p + scale_x_datetime(date_labels="%b-%Y", date_breaks = breaks) +
  theme(axis.text.x = element_text(angle = 90, hjust=1)) 
  return(p)
}

fix_date_axis <- function(p, breaks = "3 months"){
  p <- p + scale_x_date(date_labels="%b-%Y", date_breaks = breaks) +
  theme(axis.text.x = element_text(angle = 90, hjust=1)) 
  return(p)
}
...
p <- ggplot2::ggplot(plotDT, aes(x = dv_date, y = pcOfDaily, colour = Fuel_Code)) +
  geom_line() +
...
```{r hydroTileRecent, fig.cap="% solar, last 6 months"}
results <-  make_NgesoPlots(nzGenMix_wide_dt[dv_date > recentDateCut], 
                       var = "HYDRO_perc",
                       lowColour = "red",
                       highColour = "green",
...
```{r geoTile, fig.cap="% solar, all years"}
...
```{r gasTileRecent, fig.cap="% geothermal, last 6 months"}
results <-  make_NgesoPlots(nzGenMix_wide_dt[dv_date > recentDateCut], 
                       var = "GAS_perc",
                       lowColour = "red",
                       highColour = "green",
...
plotDT <- nzGenMix_dt[, .(GWh = sum(kWh/1000000, na.rm = TRUE)), 
                      keyby = .(Fuel_Code, dv_date, dv_dateTime)]
...
# calculate % contribution in each half hour
p <- ggplot2::ggplot(plotDT[dv_date > recentDateCut],
                aes(x = dv_dateTime,
                    y = pc,
                    colour = Fuel_Code)) +
  geom_line() +
...
```{r hydroVsWind, fig.cap="Comparison of hydro vs wind % contribution over time (all years)"}
...
```{r hydroVsWindRecent, fig.cap="Comparison of hydro vs wind % contribution over time (last 6 months)"}
p <- ggplot2::ggplot(plotDT[dv_dateTime > recentDateCut & (Fuel_Code == "Wind" | Fuel_Code == "Hydro")],
                aes(x = dv_dateTime,
                    y = pc,
                    colour = Fuel_Code)) +
  geom_line() +
