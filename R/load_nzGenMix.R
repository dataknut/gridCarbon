...
#'
#' `load_nzGenMix()` loads the NZ historic half-hourly wholesale generation data.
#'
...
#' @family NZ
#'
load_nzGenMix <- function(path = "~/Dropbox/data/NZ_ElecAuth/", # default
                        years = "2021", # default
                        months = seq(1,12,1),
  ...
  dataDT <- data.table::data.table() # stats collector
  for(y in years){
    ...
    for(month in months){
      ...
      if(file.exists(fullFileName)){
        # load it
        message("We already have", fullFileName, " loading...")
        dt <- data.table::fread(fullFileName)
      } else {
        # get it
      ...
      dataDT <- rbind(dataDT, dtl) # add the new file
    }
  }
  return(dataDT)
}
