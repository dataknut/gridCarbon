#' Get the GB generation data
#'
#' `get_gbGenMix()` loads the GB NG-ESO Historic Generation Mix & Carbon Intensity data.
#'
...
#' @family utils data
#'
get_gbGenMix <- function(url, dataPath, olderThan = 14){
  # load from file
  ...
  if(file.exists(lf)){
    ...
    if(dateDiff > olderThan){
      ...
      data.table::fwrite(dt, file = paste0(dataPath, "df_fuel_ckan.csv"))
        ...
        message("Most recent date in local version of data is ",
                maxDate, " (", dateDiff ," days ago)... loading")
        dt <- data.table::fread(lf)
      }
  } else{
    # no data file
